...
Battle::AbilityEffects::DamageCalcFromTarget.add(:GRASSPELT,
  proc { |ability, user, target, move, mults, power, type|
    ...
    next if !move.physicalMove?(type)
    mults[:defense_multiplier] *= 2 # Friv Buff
  }
)
...
# Forest's Curse - Additionally Leech Seeds the target
...
#===============================================================================
# 100. FOREST FIRE FIELD MECHANICS
# Hardcoded ability and move effects specific to Forest Fire Field
#===============================================================================

FORESTFIRE_FIELD_IDS = %i[forestfire].freeze

# Passive Speed increase (1.2x) for Bug types
# Applied via Battle::Battler#pbSpeed hook
class Battle::Battler
  alias forestfire_pbSpeed pbSpeed
  def pbSpeed
    speed = forestfire_pbSpeed
    return speed if !@battle.has_field? || !FORESTFIRE_FIELD_IDS.include?(@battle.current_field.id)
    return speed if pbHasType?(:BUG)
    return (speed * 1.2).round
  end
end

# Blaze - Always activated (Blaze moves 1.5x)
Battle::AbilityEffects::DamageCalcFromUser.add(:BLAZE,
  proc { |ability, user, target, move, mults, power, type|
    next if type != :FIRE
    if user.battle.has_field? && FORESTFIRE_FIELD_IDS.include?(user.battle.current_field.id)
      mults[:attack_multiplier] *= 1.5
    elsif user.hp <= user.totalhp / 3  # Normal Blaze condition
      mults[:attack_multiplier] *= 1.5
    end
  }
)

# Flare Boost - Always activated (1.5x special damage)
Battle::AbilityEffects::DamageCalcFromUser.add(:FLAREBOOST,
  proc { |ability, user, target, move, mults, baseDmg, type|
    if (user.burned? || (user.battle.has_field? && FORESTFIRE_FIELD_IDS.include?(user.battle.current_field.id))) && move.specialMove?
      mults[:base_damage_multiplier] *= 1.5
    end
  }
)

# Ingrain - Burns user and Raises Speed by +1
...
# Forest's Curse - Additionally BURN the target
class Battle::Move::AddGrassTypeToTarget < Battle::Move
  alias forestfire_pbEffectAgainstTarget pbEffectAgainstTarget if method_defined?(:pbEffectAgainstTarget)

  def pbEffectAgainstTarget(user, target)
    ret = forestfire_pbEffectAgainstTarget(user, target)
    if @battle.has_field? && FORESTFIRE_FIELD_IDS.include?(@battle.current_field.id)
        target.pbBurn(user) if target.pbCanBurn?(user,false,self)
    end
	end
end

# Wood Hammer - Additonally burns the enemy on Forest Fire Field
class Battle::Move::RecoilThirdOfDamageDealt < Battle::Move::RecoilMove

  alias forestfire_pbAdditionalEffect pbAdditionalEffect if method_defined?(:pbAdditionalEffect)

  def pbAdditionalEffect(user, target)
    if @id == :WOODHAMMER && @battle.has_field? && FORESTFIRE_FIELD_IDS.include?(@battle.current_field.id)
		   target.pbBurn(user) if target.pbCanBurn?(user,false,self)
	  end
  end
end

# Leech Seed - Additionally burns the target on Forest Fire Field
...
#===============================================================================
# 101. DARK FOREST FIELD MECHANICS
# Hardcoded ability and move effects specific to Dark Forest Field
#===============================================================================

DARK_FOREST_FIELD_IDS = %i[darkforest].freeze

# Justified - Attack boost on switch-in
Battle::AbilityEffects::OnSwitchIn.add(:JUSTIFIED,
  proc { |ability, battler, battle, switch_in|
    next if !battle.has_field? || !DARK_FOREST_FIELD_IDS.include?(battle.current_field.id)
    battler.pbRaiseStatStageByAbility(:ATTACK, 1, battler)
  }
)

# Prism Armor - Never active
...
Battle::AbilityEffects::DamageCalcFromTargetNonIgnorable.add(:SHADOWSHIELD,
  proc { |ability, user, target, move, mults, power, type|
    next !target.battle.has_field? || !DARK_FOREST_FIELD_IDS.include?(target.battle.current_field.id)
    mults[:final_damage_multiplier] /= 2 
  }
)

# Cursed Body - doubled activation chance from 30% to 60%
Battle::AbilityEffects::OnBeingHit.add(:CURSEDBODY,
  proc { |ability, user, target, move, battle|
    ...
    if battle.has_field? && DARK_FOREST_FIELD_IDS.include?(battle.current_field.id)
...
class Battle::Battler
  alias dark_forest_healing_pbRecoverHP pbRecoverHP if method_defined?(:pbRecoverHP)
  
  def pbRecoverHP(amt, anim = true)
    # Check if this is healing from Wish/Synthesis/Moonlight on Dark Forest Field
    if @battle.respond_to?(:pbGetMoveIndexFromID)
      current_move = @battle.choices[@index] ? @battle.choices[@index][2] : nil
      if current_move && @battle.has_field? && DARK_FOREST_FIELD_IDS.include?(@battle.current_field.id)
        if [:SYNTHESIS, :WISH, :MOONLIGHT].include?(current_move.id)
          # Change heal to 75%
          amt = (@totalhp * 3 / 4.0).round
        end
      end
    end
    respond_to?(:dark_forest_healing_pbRecoverHP) ? dark_forest_healing_pbRecoverHP(amt, anim) : super
  end
end

# Forest's Curse - Additionally Lock's On the target
class Battle::Move::AddGrassTypeToTarget
  alias dark_forest_pbEffectAgainstTarget pbEffectAgainstTarget if method_defined?(:pbEffectAgainstTarget)
  
  def pbEffectAgainstTarget(user, target)
    ret = dark_forest_pbEffectAgainstTarget(user, target)
    
    # On Dark Forest Field, also curse the target
    if @battle.has_field? && DARK_FOREST_FIELD_IDS.include?(@battle.current_field.id)
...
#===============================================================================
# 102. LIGHT FOREST FIELD MECHANICS
# Hardcoded ability and move effects specific to Light Forest Field
#===============================================================================

LIGHT_FOREST_FIELD_IDS = %i[lightforest].freeze

# Prism Armor - 33% increased defenses
Battle::AbilityEffects::DamageCalcFromTarget.add(:PRISMARMOR,
  proc { |ability, user, target, move, mults, power, type|
    next if !target.battle.has_field? || !LIGHT_FOREST_FIELD_IDS.include?(target.battle.current_field.id)
    # 33% defense boost = reduce damage by ~25%
    mults[:defense_multiplier] *= 1.33
  }
)

# Shadow Shield — never active
Battle::AbilityEffects::DamageCalcFromTargetNonIgnorable.add(:SHADOWSHIELD,
  proc { |ability, user, target, move, mults, power, type|
    next if target.battle.has_field? && LIGHT_FOREST_FIELD_IDS.include?(target.battle.current_field.id)
    mults[:final_damage_multiplier] /= 2 
...
# Cursed Body - halved activation chance from 30% to 15%
Battle::AbilityEffects::OnBeingHit.add(:CURSEDBODY,
  proc { |ability, user, target, move, battle|
    next if user.fainted?
    next if user.effects[PBEffects::Disable] > 0
    ...
    next if !regularMove || (regularMove.pp == 0 && regularMove.total_pp > 0)
    if battle.has_field? && LIGHT_FOREST_FIELD_IDS.include?(battle.current_field.id)
...
# Justified - +3 Attack boost on switch-in
Battle::AbilityEffects::OnSwitchIn.add(:JUSTIFIED,
  proc { |ability, battler, battle, switch_in|
    next if !battle.has_field? || !LIGHT_FOREST_FIELD_IDS.include?(battle.current_field.id)
    battler.pbRaiseStatStageByAbility(:ATTACK, 3, battler)
  }
)

# MOVE: Floral Healing — fully heals
class Battle::Move::HealTargetDependingOnGrassyTerrain
  alias lightforest_floralhealing_pbEffectAgainstTarget pbEffectAgainstTarget if method_defined?(:pbEffectAgainstTarget)

  def pbEffectAgainstTarget(user, target)
    if @id == :FLORALHEALING &&
       @battle.has_field? &&
       LIGHT_FOREST_FIELD_IDS.include?(@battle.current_field.id) &&
      # Full heal
      heal = target.totalhp - target.hp
      if heal > 0 && !target.effects[PBEffects::HealBlock]
        target.pbFieldRecoverHP(heal)
        @battle.pbDisplay(_INTL("The pure energy of the forest fully restored {1}!", target.pbThis))
      end
      return
    end
    respond_to?(:lightforest_floralhealing_pbEffectAgainstTarget) ? lightforest_floralhealing_pbEffectAgainstTarget(user, target) : super
  end
end

# Forest's Curse - Additionally starts Fairy Lock
class Battle::Move::AddGrassTypeToTarget
  alias light_forest_pbEffectAgainstTarget pbEffectAgainstTarget if method_defined?(:pbEffectAgainstTarget)
  
  def pbEffectAgainstTarget(user, target)
    ret = light_forest_pbEffectAgainstTarget(user, target)
    
    # On Light Forest Field, also creates a fairy lock
    if @battle.has_field? && LIGHT_FOREST_FIELD_IDS.include?(@battle.current_field.id)
...
Battle::AbilityEffects::DamageCalcFromTarget.add(:GRASSPELT,
  proc { |ability, user, target, move, mults, power, type|
    ...
    next if !move.physicalMove?(type)
    mults[:defense_multiplier] *= 2 # Friv boost to 2x on Grassy Terrain
  }
)
...
class Battle::Battler
  ...
  def pbFaint(showMessage = true)
    ...
    attacker = @battle.last_colosseum_killer rescue nil
